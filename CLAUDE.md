...
- `lib/features/location/` — el padre/tutor activa "compartir ubicación" y el tracking arranca directo en el celular del hijo, sin pantalla de aceptar/rechazar (`LocationCoordinator` en `location_consent.dart`) — decisión explícita del usuario, priorizando que el padre pueda ver dónde está su hijo sin depender de que acepte. El hijo sigue viendo SIEMPRE el aviso persistente de Android (indicador visible + lo que evita que Doze mate el proceso) y puede apagarlo él mismo desde Ajustes → Ubicación (`settings_screen.dart`) para ahorrar batería — si lo apaga, el coordinador respeta esa decisión y no lo vuelve a prender solo hasta que el padre reactive el switch. Tracking vía `flutter_foreground_task`. Geocercas evaluadas **en el dispositivo del hijo** (no en el Worker) para no gastar invocaciones.
