...
Zona horaria: `America/Panama`. Firebase project: `edutrack-family` (cuenta `diosdelsol266@gmail.com`). Repo GitHub bajo la cuenta `Litdemonick`.
...
- Adultos: email+password (con verificación obligatoria) o Google Sign-In. El rol (`parent`/`teacher`) se guarda como **custom claim** (para que las reglas lo lean sin lecturas extra) + espejo en `users/{uid}`. La Cloud Function `registerRole` pone el claim.
- Estudiantes **nunca se registran solos**: el padre genera un código de 6 caracteres (`createLinkCode`), el niño lo canjea (`redeemLinkCode`) y recibe un **custom token con `uid == studentId`** — así las reglas colapsan a `uid == studentId` sin necesitar un mapeo aparte.
...
- `lib/features/location/` — el hijo comparte ubicación solo con doble consentimiento (padre activa + hijo confirma en su teléfono, ver `location_consent.dart`). Tracking vía `flutter_foreground_task` (servicio con notificación persistente — es el indicador visible legal + lo que evita que Doze mate el proceso). Geocercas evaluadas **en el dispositivo del hijo** (no en Cloud Functions) para no gastar invocaciones.
- `lib/features/safety/wellness_check.dart` — check-in "¿Estás bien?": el padre dispara, el hijo responde con dos botones grandes, timeout de 3 min lo cierra la Cloud Function `checkWellnessTimeouts` (funciona aunque el padre cierre su app).
- Alerta sísmica: Cloud Function `checkSeismicActivity` sondea el feed público de USGS cada minuto (Google no tiene API de sismos) y alerta si hay un sismo cerca de la última ubicación conocida de un estudiante. Configurable por familia (`StudentProfile.seismicAlertsEnabled/seismicMinMagnitude`).

## Push

FCM puro (se quitó OneSignal — tenía la REST key filtrada en el código v1). `lib/core/services/fcm_service.dart` registra el token en `users/{uid}/devices/{installId}`. Todo push pasa por `push_queue` (Firestore) → Cloud Function `sendQueuedPush` hace el fan-out por `targetUids` con poda de tokens muertos. `NotificationBus` (`lib/core/services/notification_bus.dart`) sigue siendo el único punto de entrada — nunca dispares una notificación sin pasar por él.
...
- `functions/index.js` — las 6 Cloud Functions (roles, códigos, push, timeouts de check-in, sismos).
