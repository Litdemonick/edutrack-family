...
class _EditTaskScreenState extends ConsumerState<EditTaskScreen> {
  ...
  late int _notifDays;
  late List<String> _referenceImagePaths;
  ...
  void initState() {
    ...
    _subject = _subjects.contains(widget.task.subject)
        ? widget.task.subject
        : 'Otro';
    _category = _categories.contains(widget.task.category)
        ? widget.task.category
        : 'Otro';
    _dueDate = widget.task.dueDate;
    final maxD = _maxNotifDays(_dueDate);
    _notifDays = widget.task.notificationDaysBefore.clamp(1, maxD).toInt();
    _referenceImagePaths = List<String>.from(widget.task.referenceImagePaths);
  }
  ...
  Future<void> _addReferenceImage() async {
    ...
    final tempId = 'ref_${widget.task.id}';
  ...
  Future<void> _delete() async {
    final ok = await ConfirmationDialog.show(
      ...
      message: '¿Eliminar "${widget.task.title}"? Esta acción no se puede deshacer.',
